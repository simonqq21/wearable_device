...
void button_toggle_datalogging_cb(void *arg, void *data)
{
    if (datalogging)
    {
        ...
        if (task_handle_main_data_logging != NULL)
            xTaskNotify(task_handle_main_data_logging, CMD_DATALOGGING_STOP, eSetValueWithOverwrite);
        //  start IMU task
        if (task_handle_imu_data != NULL)
            xTaskNotify(task_handle_imu_data, CMD_IMU_STOP, eSetValueWithOverwrite);
        ...
        if (task_handle_SD_card_datalogger != NULL)
            xTaskNotify(task_handle_SD_card_datalogger, CMD_SD_CARD_STOP, eSetValueWithOverwrite);
    ...
    else
    {
        ...
        if (task_handle_main_data_logging != NULL)
            xTaskNotify(task_handle_main_data_logging, CMD_DATALOGGING_GO, eSetValueWithOverwrite);
        //  start IMU task
        if (task_handle_imu_data != NULL)
            xTaskNotify(task_handle_imu_data, CMD_IMU_READ_LOOP, eSetValueWithOverwrite);
        // turn on the status LED
        if (task_handle_status_led != NULL)
            xTaskNotify(task_handle_status_led, STATUS_LED_BLINK, eSetValueWithOverwrite);
        // start SD card datalogging
        if (task_handle_SD_card_datalogger != NULL)
            xTaskNotify(task_handle_SD_card_datalogger, CMD_SD_CARD_START, eSetValueWithOverwrite);
...
void button_trigger_calibration_cb(void *arg, void *data)
{
    ...
    if (task_handle_imu_data != NULL)
        xTaskNotify(task_handle_imu_data, CMD_IMU_CALIBRATE, eSetValueWithOverwrite);
    // vTaskDelay(1000 / portTICK_PERIOD_MS);
    // xTaskNotify(task_handle_imu, CMD_IMU_STOP, eSetValueWithOverwrite);
}
